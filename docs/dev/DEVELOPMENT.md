...
- **按钮刚出现（淡入中）时点击会被游戏丢弃**：本检测器只要求「中央有文本状的亮色带」，
  比模板匹配灵敏得多，按钮还没可点就会命中，表现为「第一次点不上、隔一会儿再点才生效」。
  **这是点击时机问题，不是检测问题**——不要用「要求连续稳定 N 帧才算命中」去解决，
  那会让每次检测都多等一截；等待只应发生在**即将点击**的那一次。

  因此检测器命中的结果 Box 带 `needs_click_delay = True` 标记，点击时据此延后：

  ```python
  BaseGameTask.confirm_click_delay = 0.1     # 默认值，调大可更稳，调 0 不等待
  self.click_confirm_target(confirm)         # 带标记才等，模板匹配命中的直接点
  ```

  `find_confirm()` 里两条路径共用这个入口：检测器命中 → 等一小会儿再点；
  模板匹配命中（能匹配上说明按钮已渲染完整）→ 立即点，不加任何延迟。
